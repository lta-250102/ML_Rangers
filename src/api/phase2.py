...
from core.phase2 import Prob1Model, Prob2Model
...
async def phase1_prob1(request: Request) -> Response:
    try:
        ...
        save_test_executor.submit(save_test, request, response, 'phase-2', 'prob-1')
        return response
...
async def phase1_prob2(request: Request) -> Response:
    try:
        model = Prob2Model()
        response = model.infer(request=request)
        save_test_executor.submit(save_test, request, response, 'phase-2', 'prob-2')
        return response
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))